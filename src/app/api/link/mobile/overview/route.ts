...
export async function GET(request: Request) {
  ...
  const [reminders, account, calendar, monitoring, arr, qbit, telegram] = await Promise.all([
    prisma.reminder.findMany({
      where: { userId, done: false },
      orderBy: { at: "asc" },
      take: 50,
      select: { id: true, title: true, at: true, repeat: true },
    }),
  ...
  return NextResponse.json({
    ...
    reminders: reminders.map((item) => ({ ...item, at: item.at.toISOString() })),
